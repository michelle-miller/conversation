...
This version of the {{site.data.keyword.conversationshort}} documentation is deprecated. Go [here](https://cloud.ibm.com/docs/services/assistant?topic=assistant-dialog-entities) instead.
{: deprecated}
...
1.  In the {{site.data.keyword.conversationshort}} tool, open your workspace and then click the **Intents** tab. If **Intents** is not visible, use the ![Menu](images/Menu_16.png) menu to open the page.
...
- Annotations associated with contexual entities are not exported. You must export the entire workspace to capture both the entity value and any associated annotations.
...
**Note**: Entity annotations are not included in the import of an entity CSV file. You must import the entire workspace to retain the associated annotations for a contextual entity in that workspace. If you export and import entities only, then any contextual entities that you exported are treated as dictionary-based entities after you import them.
